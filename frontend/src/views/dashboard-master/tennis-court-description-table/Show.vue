            ...
            <section>
                <div class="container py-5 h-100">
                    <div class="row d-flex align-items-center justify-content-center h-100">
                        ...
                        <p class="text-center"><router-link :to="{ name: 'dashboardMasterTennisCourtDescritpionTableEdit', params: { id: items.id } }"
                        ...
                        <div class="col-md-7 col-lg-5 col-xl-5 offset-xl-1">
                            <ul v-for="(item, i) in items" :key="i" class="text-start">
                                <li v-if="i !== 'cnae'
                            && i !== 'id'
                            && i !== 'name'
                            && i !== 'email_verified_at'
                            ...
                            && i !== 'slug'
                            && i !== 'description'
                            "> <strong class="text-capitalize">{{ i?.replaceAll('_', ' ') }}: </strong> {{ item }}</li>
                            </ul>
                            ...
                            <ul v-for="(itemCnae, iCnae) in items.cnae" :key="iCnae">
                                <li>
                                    <strong >{{ itemCnae.type.replace('_', ' ') }}: </strong> {{ itemCnae.name }}
                                </li>
                            </ul>
                        </div>
                    </div>
...
export default {
    name: 'DashboardMasterTennisCourtShow',
    components: {
        templateView: defineAsyncComponent(() =>
            import('@/views/dashboard-master/Template.vue')
        ),},
    data() {
        return {
            items: {},
            ...
            myUri: 'tennis-court-description-table',
        }
    },
    mounted() {
        this.getItens();
    ...
    methods: {
        async getItens() {
            const response = await Api.get(this.myUri +'/'+ this.id);
