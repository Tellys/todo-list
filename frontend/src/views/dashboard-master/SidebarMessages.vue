<template>
  <div
    class="overflow-y-auto side-navbar active-nav ml-0 d-flex justify-content-between flex-wrap flex-column shadow-lg"
    id="sidebar">
    ...
    <ul class="nav flex-column w-100 px-2 pb-5">
      <li class="d-flex flex-row justify-content-end my-2">
        <div class="btn-group" role="group" >
          ...
          <button type="button" class="btn bntShowSidebar">X</button>
        </div>

      </li>

      <li v-for="(vDados, i) in this.$store.getters['messages/itemsNotRead'].data" :key="i"
        class="nav-link w-100 list-group">
        <template v-if="vDados.id">
          <div class="list-group-item list-group-item-action p-0 m-0">
            ...
            <div @click.prevent="showMessage(vDados.id, vDados.status)" :id="'rowNotRead[' + vDados.id + ']'"
              ...
              <!-- <span :id="'isClose' + vDados.id">
                  <button type="button" class="btn me-2" @click.prevent="updateToReadMessage(vDados.id)"><i
                      class="bi bi-check2"></i></button>
                  <button type="button" class="btn me-2"><i class="bi bi-chevron-right"></i></button>
      ...
      <li v-for="(vDados, i) in this.$store.getters['messages/itemsRead'].data" :key="i"
        class="nav-link w-100 list-group">
          ...
          <div class="list-group-item list-group-item-action">
            ...
            <div @click.prevent="showMessage(vDados.id, vDados.status)"
              ...
              <div class="btn-group w-100" role="group" :id="'isClose' + vDados.id">
                ...
                <button type="button" class="btn btn-sm"><i class="bi bi-chevron-right"></i></button>
                <button type="button" :id="'isOpen' + vDados.id" class="inactive btn btn-sm"><i class="bi bi-chevron-right"></i></button>
...
export default {
  ...
  methods: {
    ...
    async list() {
      return await Api.get('message-not-read').then((response) => {
        this.items = response;
        store.commit('messages/SET_ITEMS_NOT_READ', response);
      }).then(async () => {
        await Api.get('message-read').then((response) => {
          this.items = response;
          store.commit('messages/SET_ITEMS_READ', response);
        })
      });
    },
