    ...
    <div class="d-flex justify-content-end mb-3">
        ...
        <div class="p-2 flex-grow-1">
            <form class="form" @submit.prevent="searchBy()">
                <div class="input-group shadow-lg">
                    <input type="search" v-model="register.search" required class="form-control br-0 "
                        placeholder="Pesquisar" aria-label="Pesquisar" aria-describedby="Pesquisar" name="search"
                        id="search">
                    <button @click="getItens(apiPath)"
                        class="btn btn-outline-secondary border border-0 border-top border-bottom" type="button"
                        id="btnCancelSearch"><i class="bi bi-x"></i></button>
                    <button class="btn btn-secondary border border-0 border-top border-bottom" type="submit"
                        id="btnSearch"><i class="bi bi-search"></i></button>
                </div>
            </form>
        </div>
        ...
        <div class="p-2">
            <button type="button" class="btn btn-outline-secondary" @click="getItens(atualApiPath, '&trash=true')"
                ...
                <i class="bi bi-trash"></i> Lixeira <span class="badge text-bg-danger">{{ items?.data?.meta?.trash
                    }}</span>
            </button>
                            ...
                            <div class="text-capitalize d-flex flex-row align-items-center justify-content-center">
                                ...
                                <div>
                                    <button :id="'head' + iVHeads + 'Down'"
                                        ...
                                        class="btn btn-sm p-1 " type="button"><i
                                            class="bi bi-sort-alpha-up-alt"></i></button>

                                    <button :id="'head' + iVHeads + 'Up'"
                                        @click.prevent="getItens(atualApiPath, '&oderBy[' + iVHeads + ']=desc', ['head' + iVHeads + 'Up', 'head' + iVHeads + 'Down'])"
                                        class="btn text-danger  btn-sm p-1 d-none" type="button"><i
                    ...
                    <template v-if="vDados.id">
                        ...
                        <td class="text-end">
                            <div class="btn-group" role="group" aria-label="Basic mixed styles example">
                                <button v-if="!items.data?.comandos?.restore && !vDados.check" type="button"
                                    class="btn btn-outline-secondary border-end-0" title="Check Item"
                                    @click="checkItem(vDados)"><i class="fa-solid fa-check"></i>
                                </button>

                                <button v-if="!items.data?.comandos?.restore && vDados.check" type="button"
                                    class="btn btn-outline-success border-end-0" title="Check Item"
                                    @click="checkedItem()"><i class="fa-solid fa-check-double"></i>
                                </button>

                                <button v-if="!items.data?.comandos?.restore" type="button"
                                    class="btn btn-outline-primary border-start-0" title="Visualizar Item"
                                    @click="viewItem(vDados.id)"><i class="fa-solid fa-eye"></i></button>
                                <button v-if="items.data?.comandos?.destroy" type="button" class="btn btn-danger"
                                    title="Coloar Item na Lixeira" @click="deleteItem(vDados.id)"><i
                                        class="bi bi-trash"></i></button>
                                <button v-if="items.data?.comandos?.edit" type="button"
                                    class="btn btn-outline-secondary border-start-0" title="Editar Item"
                                    @click="editItem(vDados.id)"><i class="bi bi-pencil-fill"></i></button>
                                <button v-if="items.data?.comandos?.restore" type="button" class="btn btn-outline-info"
                                    title="Reciclar Item" @click="restoreItem(vDados.id)"><i
        ...
        <nav aria-label="paginationTable">
            <ul class="pagination">
                ...
                <li class="page-item" v-if="items?.data?.db?.last_page_url">
                    <a class="page-link" href="#" @click="getItens(items?.data?.db?.last_page_url)"
                        title="Última Página">
...
export default {
    ...
    methods: {
        ...
        async editItem(id) {
            this.$router.push({ name: 'dashboardTaskEdit', params: { id: id } });
        },
        async viewItem(id) {
            this.$router.push({ name: 'dashboardTaskShow', params: { id: id } });
        },

        async checkItem(dados) {
            return await Api.update(this.myUri + '/' + dados.id, {
                 check: 'true', 
                 time_start: dados.time_start,
        ...
        async checkedItem(){
            ...
            return varMyAlertToast.toast({
                title: "Item já foi esta como concluído",
                text: '',
                icon: 'success',
            });
